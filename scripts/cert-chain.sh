...
echo ""
read -p "Enter Common Name for Root CA (e.g., MyRootCA): " CN
...
echo ""
MSYS_NO_PATHCONV=1 openssl req -x509 -new -nodes -key rootCA.key -sha256 -days 3650 -out rootCA.crt \
    -subj "/C=MK/ST=Skopje/L=Skopje/O=UKIM/OU=FINKI/CN=$CN"
...
echo ""
read -p "Enter Common Name for Intermediate CA (e.g., MyIntermediateCA): " ICN
...
echo ""
MSYS_NO_PATHCONV=1 openssl req -new -key intermediate.key -out intermediate.csr \
    -subj "/C=MK/ST=Skopje/L=Skopje/O=UKIM/OU=FINKI/CN=$ICN"
...
MSYS_NO_PATHCONV=1 openssl req -new -key localhost.key -out localhost.csr \
...
echo -e "${GREEN}✓ Server Certificate created: localhost.crt (365 days)${NC}"
echo ""
...
cat localhost.crt intermediate.crt > fullchain.crt
...
echo ""
MSYS_NO_PATHCONV=1 openssl pkcs12 -export -in fullchain.crt -inkey localhost.key \
    -name "localhost" -out fullchain.p12
...
echo -e "${BOLD}==========================================${NC}"
echo ""
echo -e "${RED}1. LOAD ROOT CERTIFICATE INTO LOCAL MACHINE:${NC}"
...
echo -e "${BLUE}   - Linux: sudo cp rootCA.crt /usr/local/share/ca-certificates/ && sudo update-ca-certificates${NC}"
echo ""
echo -e "${YELLOW}2. CERTIFICATE FILES CREATED:${NC}"
...
echo -e "${GREEN}   • localhost.key: Server private key${NC}"
echo -e "${GREEN}   • localhost.crt: Server certificate${NC}"
echo -e "${GREEN}   • fullchain.crt: Complete certificate chain${NC}"
echo -e "${GREEN}   • fullchain.p12: PKCS#12 bundle for applications${NC}"
echo ""
echo -e "${BOLD}3. USAGE IN APPLICATION:${NC}"
echo -e "${BLUE}   Copy fullchain.p12 to your application's keystore location${NC}"
echo ""
echo -e "${BOLD}==========================================${NC}"
